# Third-Party Notices

Tatoosh includes or links against the following third-party software and assets.

## vkQuake

- **Source:** https://github.com/Novum/vkQuake
- **License:** GNU General Public License v2 (GPL-2.0)
- **Location:** `engine/` (git submodule)
- **License file:** `engine/LICENSE.txt`

## RmlUI

- **Source:** https://github.com/mikke89/RmlUi
- **License:** MIT License
- **Location:** `external/rmlui/` (git submodule)
- **License file:** `external/rmlui/LICENSE.txt`

## LibreQuake

- **Source:** https://github.com/lavenderdotpet/LibreQuake
- **License:** BSD License
- **Location:** `external/librequake/` (git submodule)
- **License file:** `external/librequake/docs/COPYING`

## Lato Font

- **Source:** http://www.latofonts.com/
- **Author:** Lukasz Dziedzic
- **License:** SIL Open Font License 1.1 (OFL-1.1)
- **Location:** `ui/fonts/LatoLatin-*.ttf`
- **License file:** `ui/fonts/LICENSE.txt`

## Space Grotesk Font

- **Source:** https://fonts.google.com/specimen/Space+Grotesk
- **Author:** Florian Karsten
- **License:** SIL Open Font License 1.1 (OFL-1.1)
- **Location:** `ui/fonts/SpaceGrotesk-Bold.ttf`
- **License file:** https://github.com/floriankarsten/space-grotesk/blob/master/OFL.txt

## Open Sans Font

- **Source:** https://fonts.google.com/specimen/Open+Sans
- **Author:** Steve Matteson / Google Fonts
- **License:** SIL Open Font License 1.1 (OFL-1.1)
- **Location:** `ui/fonts/OpenSans.ttf`
- **License file:** `ui/fonts/LICENSE.txt`
