...
3. **Composite** — Premultiplied alpha blend: `game * (1 - ui.a) + ui.rgb`

4. **Color grading** — Contrast multiply, then gamma power curve.

## Push Constants

All effect parameters are passed per-frame via Vulkan push constants from `GL_EndRenderingTask()` in `engine/Quake/gl_vidsdl.c`.
...
| 24 | `ui_opacity` | `scr_sbaralpha` cvar | UI layer opacity (0 = invisible, 1 = fully opaque) |
...
The HUD layer shifts slightly in response to player movement, computed per-frame in `V_UpdateHudInertia()` (`engine/Quake/view.c`). Two independent axes:
...
| File | Role |
|------|------|
| `engine/Shaders/postprocess.vert` | Fullscreen triangle vertex shader |
| `engine/Shaders/postprocess.frag` | Compositing + effects fragment shader |
| `engine/Quake/gl_vidsdl.c` | Vulkan resource setup, descriptor sets, push constant dispatch |
| `engine/Quake/view.c` | `V_UpdateHudInertia()` — spring physics for HUD offset |
| `engine/Quake/view.h` | Exports `v_hud_offset_x`, `v_hud_offset_y` |
